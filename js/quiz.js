...
async function initQuiz() {
    ...
    if (quizTitleEl) {
        quizTitleEl.textContent = (subjectNames[quizSubject] || 'Subject') + ' Quiz';
    }
    ...
    try {
        ...
        if (!response) {
            console.error('Could not find quiz.json');
            showNoQuizMessage();
            return;
        }
...
// FIX: Properly expose logout to window with auth available
