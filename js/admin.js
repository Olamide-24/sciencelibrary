...
// Format input as they type (auto-capitalize)
searchInput.addEventListener('input', (e) => {
    let value = e.target.value.toUpperCase();
    // Auto-format: SCI-YYYY-XXXX
    value = value.replace(/[^SCI0-9-]/g, '');
...
window.searchUser = async function() {
    const regId = searchInput.value.trim();
    
    if (!regId || regId.length < 8) {
        searchError.querySelector('span').textContent = 'Please enter a valid Registration ID';
